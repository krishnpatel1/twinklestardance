...
class AgerangeController extends Controller
{
	...
	public function actionDelete($id)
	{
        ...
        if (Yii::app()->getRequest()->getIsPostRequest()) {
            $this->loadModel($id, 'Agerange')->delete();
	...
	{                            
        // FOR DELETE SELECTED CATEGORIES //
        if (Yii::app()->getRequest()->getIsPostRequest()) {
            $anDeletedIds = isset($_POST['deleteids']) ? $_POST['deleteids'] : array();
            if (count($anDeletedIds) > 0) {
                $bDeleted = Agerange::removeSelectedItems($anDeletedIds);
                if ($bDeleted) {
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='agerange-form')
		{
