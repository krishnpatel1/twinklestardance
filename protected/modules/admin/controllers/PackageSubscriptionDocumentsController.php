...
class PackageSubscriptionDocumentsController extends AdminCoreController {
        ...
        if (Yii::app()->request->isPostRequest) {
            ...
            if ($oModel->validate()) {
                ...
                if ($oModel) {
                    ...
                    $oDocModel->subscription_id = $_POST['PackageSubscriptionDocuments']['subscription_id'];
                    $oDocModel->save();
                }
    ...
    public function actionUpdate($id) {
        $oModel = $this->loadModel($id, 'PackageSubscriptionDocuments');
        ...
        if (Yii::app()->request->isPostRequest) {
            ...
            $amPostData = $_POST['PackageSubscriptionDocuments'];
    ...
    public function actionIndex() {
        $dataProvider = new CActiveDataProvider('PackageSubscriptionDocuments');
        ...
        if (AdminModule::isAdmin()) {
            ...
            if (AdminModule::isStudioAdmin()) {
                ...
                if (count($anSubIds) == 0) {
                    ...
                    $oCriteria->select = 'cd.*';
                    $oCriteria->alias = 'cd';
                    $oCriteria->join = 'INNER JOIN package_subscription_documents_transaction psdt ON cd.document_id = psdt.document_id';
                    $oCriteria->join .= ' INNER JOIN classes c ON c.id = cd.class_id';
                    $oCriteria->condition = 'c.created_by =:snUserID AND psdt.subscription_id IN (' . implode(',', $anSubIds) . ')';
                    $oCriteria->params = array(':snUserID' => Yii::app()->admin->id);
                    $oCriteria->group = 'cd.id';
                }
            } else {    // FOR INSTRUCTOR //
                $oCriteria = new CDbCriteria();
                $oCriteria->alias = 'cd';
