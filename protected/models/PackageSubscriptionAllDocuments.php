...
Yii::import('application.models._base.BasePackageSubscriptionAllDocuments');

class PackageSubscriptionAllDocuments extends BasePackageSubscriptionAllDocuments {
    ...
    public function validateDocSelection($attribute, $params) {
        if ($_FILES['PackageSubscriptionAllDocuments']['name']['document_name'] == '' && $_POST['PackageSubscriptionAllDocuments']['document_url'] == '')
            $this->addError('document_name', 'Please select a file or provide external url !!!');
        if (!strstr($_POST['PackageSubscriptionAllDocuments']['document_url'], 'http') && $_POST['PackageSubscriptionAllDocuments']['document_url'] != '')
