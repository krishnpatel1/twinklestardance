...
class AdminBreadcrumbs extends CWidget {
    ...
    public function run() {
        ...
        $links = array();
        $links[] = $this->homeLink = CHtml::link('Dashboard' . "<span>&nbsp;</span>", AdminModule::getUrl('home'));

        if (count($this->links) > 0) {
            foreach ($this->links as $label => $url) {
                if (isset($url['url']) && (is_string($label) || is_array($url['url']))) {
                    $links[] = CHtml::link($label, $url['url'], array('class' => $url['class']));
                } else {
                    $links[] = '<span class="' . $url['class'] . '">' . $url['label'] . '</span>';
                }
            }
        }
        echo implode("", $links);
