...
if($_REQUEST['action']=='addTocart')
{
	...
	if(trim($_POST['pack_price'])=="") {$obj->add_message("message","Choose Package Price!");}
	if($obj->get_message("message")=="")
	{
		$price=$_POST['pack_price'];
		$prod_id=$_POST['id'];
		$p_type=$_POST['p_type'];
		header("Location:add_to_cart.php?prod_id=$prod_id&price=$price&p_type=$p_type");
	}
}
?>
          ...
          <tr>
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
              ...
              <tr>
				...
				<ul>
					<li>
						<input type="radio" name="pack_price" value="<?=$package_fetch['package_price_subscription']?>">
						STARS monthly payment package <span>$<?=$package_fetch['package_price_subscription']?></span> per month for <?=$package_fetch['package_duration']?> Months
					</li>
					<li>
						<input type="radio" name="pack_price" value="<?=$package_fetch['package_price_onetime']?>">
						STARS Pay up front package - <span> $<?=$package_fetch['package_price_onetime']?></span>
					</li>
				</ul>	
				<ul>
