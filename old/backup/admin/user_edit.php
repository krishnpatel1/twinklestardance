...
adminSecure();
$user_id=$_REQUEST['id'];
$user_pid=$_REQUEST['user_pid'];
$return_url=urldecode($_REQUEST['return_url']);
if($user_pid=='')
$return_url=$return_url?$return_url:'user_management.php';
else
$return_url=$return_url?$return_url:'list_sub_user.php?user_id='.$user_pid;
...
if($_POST['btnSubmit']=='Save')
{
	...
	if(!$err)
	{
		if($user_id)
		{
			$_POST['user_password']=$_REQUEST['password'];
...
function selectFields(val,uid)
{
	jQuery.ajax({
	type: "POST",
	url: "ajax_fields.php",
	data: "countries_id="+val+"&uid="+uid,
	success: function(msg){
			$('#field').html(msg);
		}
	});
...
$(document).ready(function() {
	<?php if($data['user_country']){?>
		selectFields('<?=$data['user_country']?>','<?=$user_id?>');
	<? } ?>
            ...
            <div class="hor-scroll">
              ...
              <form action="" method="post" enctype="multipart/form-data" name="frm" id="frm">
				...
				<table width="100%" border="0" cellpadding="5" cellspacing="0" class="tableborder_gray">
				  ...
				  <tr class="box-bg">
                    <td width="20%" align="left" valign="middle" class="bodytext">Studio Name:</td>
                    <td align="left" valign="middle" class="bodytext"><input name="user_studio_name" type="text" class="field_gray" id="user_studio_name" value="<?=$data['user_studio_name'];?>" size="40"/></td>
                  </tr>
				  ...
				  <tr class="box-bg">
                    ...
                    <td align="left" valign="middle" class="bodytext" id="state">
						<select name="user_country"  id="user_country" class="input" style="width:230px;" onchange="selectFields(this.value,'<?=$user_id?>')">
					...
					<td colspan="2">
						<div id="field">
							<table align="left">
								...
								<tr>
									<td width="20%" align="left" valign="middle" class="bodytext"><label>State</label></td>
								...
								<tr>
									<td width="20%" align="left" valign="middle" class="bodytext"><label>Zip/Postal Code</label></td>
				  ...
				  <?php /*?><tr class="box-bg">
					...
					<input name="user_state" type="text" class="field_gray" id="user_state" value="<?=$data['user_state'];?>" size="40"/><span style="color:#FF0000;">*</span>
					</td>
                  </tr>
				  <tr class="box-bg">
				 ...
				 <?php /*?> <tr>
                    <td width="20%" align="left" valign="middle" class="bodytext">Select Package :</td>
                    <td align="left" valign="middle" class="bodytext">
						<?php echo $obj->getPackageList('package_id',$data['package_id']);?>
					</td>
                  </tr><?php */?>
				  <tr class="box-bg">
                    <td width="20%" align="left" valign="middle" class="bodytext">Password:</td>
                    <td align="left" valign="middle" class="bodytext"><input name="password" type="text" class="field_gray" id="password" value="<?=$data['user_password'];?>" size="40"/><span style="color:#FF0000;">*</span></td>
                  </tr>
				  <tr class="box-bg">
                    <td width="20%" align="left" valign="middle" class="bodytext">User Image :</td>
                    <td align="left" valign="middle" class="bodytext"><input type="file" name="user_pic" /></td>
                  </tr>
				  <? if($data['user_pic']!=""){?>
				  <tr class="box-bg">
					  <td height="30" align="left" valign="top" class="blacktxt_bold">&nbsp;</td>
					  <td align="left" valign="middle">
					 <?=$obj->getImageThumb(USER_IMG,$data['user_pic'],'','','100','100','../');?>
					 </td>
                  </tr>
				  <? }?>
				   <tr class="box-bg">
                    <td width="20%" align="left" valign="middle" class="bodytext">Social Media Link:</td>
                    <td align="left" valign="middle" class="bodytext"><input name="user_media_link" type="text" class="field_gray" id="user_media_link" value="<?=$data['user_media_link'];?>" size="40"/></td>
                  </tr>
				  <tr class="box-bg">
