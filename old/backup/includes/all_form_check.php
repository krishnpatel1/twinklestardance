...
if(isset($_POST['ForgotPass']))
{
	...
	{
		$user = emailUsed(trim($_POST['user_email']));		
		if($user)
		{
...
if(isset($_POST['confirm_order']))
{
	...
	if(trim($_POST['order_country'])=="") {$obj->add_message("message","Country Should Not Be Blank!");}
	
	if($_POST['opt']==1 || $_POST['opt']==0)
	{
		if(trim($_POST['order_street'])=="") {$obj->add_message("message","Owner Street Address Should Not Be Blank!");}
		if(trim($_POST['order_city'])=="") {$obj->add_message("message","Owner City Should Not Be Blank!");}
		if(trim($_POST['order_state'])=="") {$obj->add_message("message","Owner Province Should Not Be Blank!");}
		if(trim($_POST['order_zip'])=="") {$obj->add_message("message","Owner Postal Code Should Not Be Blank!");}
		
		if(trim($_POST['order_bill_street'])=="") {$obj->add_message("message","Billing Street Address Should Not Be Blank!");}
		if(trim($_POST['order_bill_city'])=="") {$obj->add_message("message","Billing City Should Not Be Blank!");}
		if(trim($_POST['order_bill_state'])=="") {$obj->add_message("message","Billing Province Should Not Be Blank!");}
		if(trim($_POST['order_bill_zip'])=="") {$obj->add_message("message","Billing Postal Code Should Not Be Blank!");}
	}
	if($_POST['opt']==2)
	{
		if(trim($_POST['order_street'])=="") {$obj->add_message("message","Owner Street Address Should Not Be Blank!");}
		if(trim($_POST['order_city'])=="") {$obj->add_message("message","Owner City Should Not Be Blank!");}
		if(trim($_POST['order_state'])=="") {$obj->add_message("message","Owner State/Territory Should Not Be Blank!");}
		
		if(trim($_POST['order_bill_street'])=="") {$obj->add_message("message","Billing Street Address Should Not Be Blank!");}
		if(trim($_POST['order_bill_city'])=="") {$obj->add_message("message","Billing City Should Not Be Blank!");}
		if(trim($_POST['order_bill_state'])=="") {$obj->add_message("message","Billing State/Territory Should Not Be Blank!");}
	}
	if($_POST['opt']==3)
	{
		if(trim($_POST['order_company'])=="") {$obj->add_message("message","Owner Company Name Should Not Be Blank!");}
		if(trim($_POST['order_bulding'])=="") {$obj->add_message("message","Owner Building Name Should Not Be Blank!");}
		if(trim($_POST['order_building_no'])=="") {$obj->add_message("message","Owner Builidig Number Should Not Be Blank!");}
		if(trim($_POST['order_locality'])=="") {$obj->add_message("message","Owner Locality Should Not Be Blank!");}
		if(trim($_POST['order_post'])=="") {$obj->add_message("message","Owner Post Should Not Be Blank!");}
		if(trim($_POST['order_state'])=="") {$obj->add_message("message","Owner State Should Not Be Blank!");}
		if(trim($_POST['order_city'])=="") {$obj->add_message("message","Owner Town Should Not Be Blank!");}
		if(trim($_POST['order_zip'])=="") {$obj->add_message("message","Owner Post Code Should Not Be Blank!");}
		
		if(trim($_POST['order_bill_company'])=="") {$obj->add_message("message","Billing Company Name Should Not Be Blank!");}
		if(trim($_POST['order_bill_bulding'])=="") {$obj->add_message("message","Billing Building Name Should Not Be Blank!");}
		if(trim($_POST['order_bill_building_no'])=="") {$obj->add_message("message","Billing Builidig Number Should Not Be Blank!");}
		if(trim($_POST['order_bill_locality'])=="") {$obj->add_message("message","Billing Locality Should Not Be Blank!");}
		if(trim($_POST['order_bill_post'])=="") {$obj->add_message("message","Billing Post Should Not Be Blank!");}
		if(trim($_POST['order_bill_state'])=="") {$obj->add_message("message","Billing State Should Not Be Blank!");}
		if(trim($_POST['order_bill_city'])=="") {$obj->add_message("message","Billing Town Should Not Be Blank!");}
		if(trim($_POST['order_bill_zip'])=="") {$obj->add_message("message","Billing Post Code Should Not Be Blank!");}
	}
	...
	{
		if($obj->is_exist(trim($_POST['order_business_email']))>0) {$obj->add_message("message","Business Email Already Exist ! Please Log In");}
	}
	
	/*if(trim($_POST['order_card'])=="") {$obj->add_message("message","Credit Card Number Should Not Be Blank!");}
	if(trim($_POST['order_card_type'])=="") {$obj->add_message("message","Credit Card Type Should Not Be Blank!");}
	if(trim($_POST['order_ex_month'])=="") {$obj->add_message("message","Card Expiry Month Should Not Be Blank!");}
	if(trim($_POST['order_ex_year'])=="") {$obj->add_message("message","Card Expiry Year Should Not Be Blank!");}
	if(trim($_POST['order_cvv'])=="") {$obj->add_message("message","Credit Card CVV Should Not Be Blank!");}*/
	
	//isset($_POST['is_agree'])?$_POST['is_agree']='Yes':$_POST['is_agree']='No';
	
	if($obj->get_message("message")=="")
	{
		isset($_POST['order_free_sub'])?$_POST['order_free_sub']='Yes':$_POST['order_free_sub']='No';
		isset($_POST['order_send_inf'])?$_POST['order_send_inf']='Yes':$_POST['order_send_inf']='No';
		isset($_POST['order_opt_mail'])?$_POST['order_opt_mail']='Yes':$_POST['order_opt_mail']='No';
		
		$_SESSION['order'] = $_POST;
		$_SESSION['messageClass'] = 'successClass';
		unset($_SESSION['subscription']);
		//$obj->reDirect("http://192.168.1.100:82/projects/2012/twinkle_star_dance/codes/confirm.php");
		$obj->reDirect(FURL_HTTPS."confirm.php");
	}
}
///////////////////////////////Post Event//////////////////////////////////////
